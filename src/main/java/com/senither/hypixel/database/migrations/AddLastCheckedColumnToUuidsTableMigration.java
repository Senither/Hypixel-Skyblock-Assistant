...
public class AddLastCheckedColumnToUuidsTableMigration implements Migration {
    @Override
    public boolean up(DatabaseManager databaseManager) throws SQLException {
        return databaseManager.queryUpdate(
            "ALTER TABLE `uuids` ADD `last_checked_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP;"
        );
    }
    ...
    public boolean down(DatabaseManager databaseManager) throws SQLException {
        return databaseManager.queryUpdate(
            "ALTER TABLE `uuids` DROP `last_checked_at`;"
        );
    }
}

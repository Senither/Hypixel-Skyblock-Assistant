...
public class AddSplashesColumnsToGuildsTableMigration implements Migration {

    @Override
    public boolean up(DatabaseManager databaseManager) throws SQLException {
        return databaseManager.queryUpdate(
            "ALTER TABLE `guilds`\n" +
                "    ADD `splash_channel` BIGINT UNSIGNED NULL AFTER `auto_rename`,\n" +
                "    ADD `splash_role` BIGINT UNSIGNED NULL AFTER `splash_channel`;"
        );
    }
    ...
    public boolean down(DatabaseManager databaseManager) throws SQLException {
        return databaseManager.queryUpdate(
            "ALTER TABLE `guilds` DROP `splash_channel`, `splash_role`;"
        );
    }
}

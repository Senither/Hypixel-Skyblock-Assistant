...
public class AddLastDonatedAtColumnToDonationsTableMigration implements Migration {
    ...
    public boolean up(DatabaseManager databaseManager) throws SQLException {
        return databaseManager.queryUpdate(
            "ALTER TABLE `donation_points`\n" +
                "    ADD `last_donated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP AFTER `last_checked_at`;"
        );
    }
    ...
    public boolean down(DatabaseManager databaseManager) throws SQLException {
        return databaseManager.queryUpdate(
            "ALTER TABLE `donation_points` DROP `last_donated_at`;"
        );
    }
}

...
import com.senither.hypixel.contracts.blacklist.PunishmentLevel;
...
public class Ratelimit {
        ...
        user.openPrivateChannel().queue(channel -> {
            channel.sendMessage(MessageFactory.createEmbeddedBuilder()
                ...
                .setDescription("Looks like you're using commands a bit too fast, I've banned you "
                    + "from using any commands until you cool down a bit.\n"
     ...
     *
     * @param id The ID of the user that should hit the ratelimit.
     ...
     */
    @Nullable
    public Carbon hit(long id) {
        Rate rate = cache.getUnchecked(id);
        if (rate == null) {
        ...
        log.info("{} has been added to the blacklist for excessive command usage, the blacklist expires {}.",
            id, punishment.toDayDateTimeString()
        );

        blacklist.addIdToBlacklist(id, "Automatic blacklist due to excessive command usage.", punishment);
